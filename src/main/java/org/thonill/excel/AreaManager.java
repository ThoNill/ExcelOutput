 ...
 * Areas represent ranges of cells defined by a name.
 * Cells represent single cell values defined by a name.
 * 
 * The main functions are:
 * - Constructor: Initializes areas and cells from workbook names
 * - write(): Writes the areas and cells back to the workbook
 * 
 * It contains utility functions to:
 * - Add a default area if no areas are defined
 * - Add areas and cells based on names
 * - Search for an area name if none are found
 */
public class AreaManager {
    ...
    private void addArea(Workbook workbook, Name name, ArrayValue arrayValue) {
        if (name.getSheetIndex() >= 0) {
            LOG.info(
                    "Name: " + name.getNameName() + " " + name.getSheetIndex() + " " + arrayValue.toString());
