...
public class AusgabeSteuerItem {
    ...
    /**
     * Constructor for AusgabeSteuerItem.
     * Initializes the object's fields from the provided data map.
    ...
    /**
     * Gets a value from the given data map for the specified key.
     * Throws an exception if the key is not found and b is true.
     * Returns an empty string if the key is not found and b is false.
     */
    ...
    public void createAusgabeDatei(Connection conn) throws Exception {
        ...
        List<RawSqlStatement> rawStatements = RawSqlStatement
                .getRawSqlStatements(sqlDatei);
    ...
    /**
     * Creates output files for each AusgabeSteuerItem in the given list by
     * calling createAusgabeDatei on each one.
     * 
     ...
     * steuerDatei Excel file.
     * Reads the steuerItems using ReadSteuerItems and then calls
     * createAusgabeDateien to generate
     * the output for each one, using the provided database connection.
     *
