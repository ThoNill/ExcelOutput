 ...
 * database.
 * It handles executing the statement, checking if it is a SELECT statement,
 * validating
 * that it can be prepared, and exporting result sets to Excel.
 */
...
public class ExecutableStatement implements ArrayValue {
     ...
     * boolean isResultSet = stmt.execute(this.query);
     * if (isResultSet) {
     * try (ResultSet rs = stmt.getResultSet()) {
     * writeCsvExcel(ausgabeDatei, vorlageDatei, rs);
     * }
     * }
     * }
     * 
